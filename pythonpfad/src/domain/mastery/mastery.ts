 ...
 *  - **Versioniert.** Jeder gespeicherte Wert trägt die Version des Algorithmus,
 *    mit dem er entstanden ist. Eine spätere Änderung entwertet alte Daten
 *    dadurch nicht stillschweigend.
 *  - **Konfigurierbar.** Alle Zahlen stehen in `DEFAULT_MASTERY_CONFIG` und
 *    lassen sich für Experimente überschreiben, ohne die Logik anzufassen.
 *  - **Nicht diskriminierend.** In die Berechnung fließen ausschließlich
 *    Merkmale der Bearbeitung ein (Korrektheit, Eigenständigkeit,
 *    Transferleistung, Fehlerart, Abstand zur letzten Übung). Keine
 *    demografischen Merkmale, keine Bearbeitungsgeschwindigkeit als
 *    Leistungsmaß – langsames Arbeiten ist kein Defizit.
 *
 * Der Wert ist eine Orientierung für die Planung, keine Messung einer Person.
 * Wie er im UI kommuniziert wird, steht in `describeMastery()`.
 */
...
export interface MasteryConfig {
  ...
  /** Rückstufung bei reinem Syntax-/Einrückungsfehler. */
  syntaxErrorPenalty: number;
  /** Rückstufung bei Konzept- oder Logikfehler. */
  conceptErrorPenalty: number;
...
export const DEFAULT_MASTERY_CONFIG: MasteryConfig = {
  ...
  syntaxErrorPenalty: 2,
  conceptErrorPenalty: 7,
...
export interface MasteryUpdate {
  ...
  delta: number;
  /**
   * Für Menschen lesbare Begründung der Veränderung. Wird im
   * Fortschrittsbereich angezeigt, damit der Wert nachvollziehbar bleibt.
   */
  reasons: string[];
...
/**
 * Aufgaben mit weniger Vorgaben belegen mehr. Eine Lückenaufgabe (Stufe 3)
 * zeigt weniger als eine frei geschriebene Lösung (Stufe 5).
 */
...
const TRANSFER_TYPES: ReadonlySet<ExerciseTypeName> = new Set(['TRANSFER', 'MINI_PROJECT']);
...
export function updateMastery(
  ...
  // --- Misserfolg -----------------------------------------------------------
  const isSurfaceError = evidence.errorType === 'SYNTAX' || evidence.errorType === 'INDENTATION';
  let penalty = isSurfaceError ? config.syntaxErrorPenalty : config.conceptErrorPenalty;

  if (isSurfaceError) {
    reasons.push(
      'Der Fehler liegt in der Schreibweise, nicht im Konzept. Das wirkt sich nur gering auf den Kompetenzwert aus.',
    );
  } else {
...
/**
 * Ordnet einen Wert einem Band zu.
 *
 * Wichtig: Nach außen wird das Band kommuniziert, nicht die Nachkommastelle.
 * Ein Wert von 82 ist keine objektive Messung menschlicher Fähigkeit, und die
 * Oberfläche stellt ihn auch nicht so dar.
 */
...
const DESCRIPTIONS: Record<MasteryBand, MasteryDescription> = {
  ...
  solid: {
    ...
    nextStep: 'Verknüpfe es in Projekten mit anderen Konzepten.',
  },
  durable: {
    band: 'durable',
...
export interface CalibrationSummary {
  /** Anzahl ausgewerteter Bearbeitungen mit Selbsteinschätzung. */
  samples: number;
  /** Durchschnittliche Sicherheit 0–1. */
  averageConfidence: number;
  /** Tatsächliche Erfolgsquote 0–1. */
  actualSuccessRate: number;
  /** Positiv = Selbstüberschätzung, negativ = Unterschätzung. */
  gap: number;
  /** Behutsam formulierte Einordnung für die Oberfläche. */
  message: string;
}
...
export function summarizeCalibration(
  ...
  if (entries.length < 5) {
    ...
    message =
      'Deine Einschätzung liegt öfter über dem Ergebnis. Das ist normal und gut zu beheben: Prüfe vor dem Absenden noch einmal einen konkreten Beispielfall im Kopf durch.';
  } else if (gap < -0.2) {
    message =
      'Du löst mehr richtig, als du dir zutraust. Deine Einschätzung darf ruhig etwas selbstbewusster ausfallen.';
  } else {
    message =
      'Deine Einschätzung und dein Ergebnis passen gut zusammen. Das ist eine hilfreiche Fähigkeit beim Programmieren.';
  }
