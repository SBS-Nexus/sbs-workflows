...
/**
 * Einfache Ratenbegrenzung mit gleitendem Fenster.
 *
 * Bewusst im Arbeitsspeicher gehalten: Für eine Instanz genügt das, und es
 * kommt keine weitere Abhängigkeit (Redis) hinzu. Wird die Anwendung auf
 * mehrere Instanzen verteilt, muss dieser Speicher gegen einen gemeinsamen
 * Zähler getauscht werden – die Schnittstelle bleibt dabei gleich. Vermerkt in
 * docs/SICHERHEIT.md.
 */
...
export interface RateLimitResult {
  allowed: boolean;
  remaining: number;
  /** Sekunden bis zum nächsten freien Versuch. */
...
export const RATE_LIMITS = {
  ...
  tutor: { limit: 30, windowMs: 60 * 60 * 1000 },
...
export function checkRateLimit(
  ...
  // Gelegentliches Aufräumen, damit der Speicher nicht unbegrenzt wächst.
