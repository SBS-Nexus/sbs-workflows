import type { ConfidenceLevel, ErrorCategoryName, MasteryState } from '@/domain/mastery/mastery';
 ...
 *
 * Bearbeitungsdauer geht bewusst NICHT ein. Kuratierte, modulübergreifende
 * Wiederholungssets (ReviewSet) sind in dieser Ausbaustufe nicht modelliert
 * (siehe docs/LEHRPLAN.md) – dieses Modul deckt nur die Planung einzelner
 * Aufgaben.
 */
...
export function scheduleNextReview(input: ScheduleInput): ScheduleResult {
  ...
  if (!passed) {
    const isSurfaceError = errorType === 'SURFACE';
    // Ein übersehenes Detail setzt den Lernstand nicht komplett zurück.
    ...
    return {
      ...
      reason: isSurfaceError
        ? 'Der Fehler lag in einem übersehenen Detail. Eine kurze Auffrischung morgen genügt.'
 ...
 *
 * Regeln:
 *  1. Überfällige Aufgaben zuerst, danach die mit dem niedrigsten
 *     Kompetenzstand.
 ...
 *     unterschiedliche Konzepte eingeführt sind, wird nicht gemischt.
 */
